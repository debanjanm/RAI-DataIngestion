...
for paper in client.results(search):
    metadata = {
        ...
        "published": paper.published.formatted(),
        "id": paper.get_short_id()
    }
    pdf_url = paper.pdf_url
    r = requests.get(pdf_url)
    with open(f"{metadata['id']}.pdf", "wb") as f:
        f.write(r.content)
    with open(f"{metadata['id']}.json", "w") as f:
        import json; json.dump(metadata, f, indent=2)
            ...
            if pdf_file.endswith(".pdf"):
                ...
                if not os.path.exists(md_path):
                    try:
                        ...
                        snippet = output_md[:500] + "..." if len(output_md) > 500 else output_md
                        self.save_markdown(snippet, md_path)
